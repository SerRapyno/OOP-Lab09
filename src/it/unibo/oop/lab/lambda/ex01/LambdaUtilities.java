...
public final class LambdaUtilities {
	 ...
	 */
	  ...
	  public static <R, T> Map<R, Set<T>> group(final List<T> list, final
	  Function<T, R> op) {
	  ...
	  list.forEach(t -> {
		  m.merge(op.apply(t), new HashSet<T>(Arrays.asList(t)), (newset, oldset) -> { 
			  newset.addAll(oldset); return newset; });
	  });

	  /*
	  list.forEach(t -> {
		  m.merge(op.apply(t), t, m.values().add(t));
	  });
	  */
